...
func solution(_ new_id:String) -> String {
    ...
    for item in getId
    {
        if item.isLetter || item.isNumber || item == "-" || item == "_" || item == "."
        {
            recommendID.append(item)
        }
    }
    ...
    while recommendID.hasPrefix(".") {
        recommendID.removeFirst()
    }
    
    while recommendID.hasSuffix(".") {
        recommendID.removeLast()
    }
    
    // MARK: 5단계 new_id가 빈 문자열이라면, new_id에 "a"를 대입합니다.
    if recommendID.count == 0 {recommendID.append("a")}
    
    // MARK: 6단계 new_id의 길이가 16자 이상이면, new_id의 첫 15개의 문자를 제외한 나머지 문자들을 모두 제거합니다. 만약 제거 후 마침표(.)가 new_id의 끝에 위치한다면 끝에 위치한 마침표(.) 문자를 제거합니다.
    while recommendID.count >= 16 {
        recommendID.removeLast()
    }
